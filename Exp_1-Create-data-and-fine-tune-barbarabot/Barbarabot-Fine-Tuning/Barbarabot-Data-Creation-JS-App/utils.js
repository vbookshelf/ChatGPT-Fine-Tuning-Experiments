...
function saveChatHistoryToCsv() {
  ...
  // message_list contains the chat history
  // For the fine tuning data creation the role and content are converted into strings
  //message_list.forEach((message) => {
    //rows.push([message.role, message.content]);
  //});
  
  
  // Remove html tags and leading and trailing spaces
	function removeHTMLTags(html) {
	    return html.replace(/<[^>]*>/g, '').trim();
	}
	
	// Add quotation marks.
	// If there are no quotation marks then
	// the multiple commas in a line will create an error
	// when loading the csv file using Pandas.
	message_list.forEach((message) => {
	    const role = '"' + String(message.role) + '"';
	    //const content = '"' + removeHTMLTags(String(message.content)) + '"';
		const content = '"' + String(message.content) + '"';
		
		// Don't save the system message because
		// the long barbarabot system message was causing the csv file
		// to be truncated.
  ...
  //message_list.forEach((message) => {
    //rows.push(['"' + String(message.role) + '"', '"' + String(message.content) + '"']);
	//});
